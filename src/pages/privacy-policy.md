...
- OpenStreetMap (Nominatim). We use OpenStreetMap's Nominatim geocoding service to name regions. When a new Gym Zone is created, our server looks up a human-readable name for the area (for example, "Greater Austin Area") by sending the center point of that area, never your own position. When you change your region by typing a city or zip code, our server sends that text to find your general area. These lookups are made from our server, not your device, and carry no account identifier: never your user ID, email, username, or name. See Section 3.3 for what we keep.
