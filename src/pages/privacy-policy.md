...
- Approximate location (optional, off by default). If you enable location, your device first snaps the GPS fix to a grid of roughly 150 meters as a temporary on-device step, then converts it into a coarse area code covering an area roughly 5 kilometers across. Only that coarse area code is sent to and stored on our servers. We use it to place you in a regional Gym Zone and in the national Gym Zone for its country, to run Gym Zone leaderboards, and to choose which partner business cards you see (see Section 5.4). To name a regional Gym Zone, our server looks up the center point of that area (never your exact position) with OpenStreetMap's Nominatim geocoding service; the lookup is made from our server, not your device, and carries no account identifier. Anyone placed in a regional Gym Zone is also placed in the worldwide Gym Zone, which is the same for every player. Once you have been placed in a regional Gym Zone, the app stops sending this area code; no location signal leaves your device again unless you need to be placed in a zone again. Your exact GPS coordinates are never stored or transmitted, battle matchmaking never uses your location, and no business or advertiser ever receives your location from us.
- Region selection. When you set or change your Gym Zone region by typing a city or zip code, our server sends the text you enter to OpenStreetMap's Nominatim geocoding service to find your general area; if you enter a zip code made up only of digits and you are already in a regional Gym Zone, it also sends that zone's two-letter country code, so the zip code is matched in your country first. The lookup is made from our server, not your device, and carries no account identifier. We store only the resulting approximate zone with your account, never the city or zip you typed. So that the same search is not sent to OpenStreetMap again, our server keeps a one-way coded fingerprint of the search text (never the text itself) together with the approximate area it matched, kept apart from your account, with no user ID or other identifier, for up to 30 days.
...
- OpenStreetMap (Nominatim). We use OpenStreetMap's Nominatim geocoding service to name regions. To give a Gym Zone a human-readable name (for example, "Greater Austin Area"), our server sends the center point of that area, never your exact position. When you change your region by typing a city or zip code, our server sends that text (and, for a zip code made up only of digits, the two-letter country code of your current regional Gym Zone) to find your general area. These lookups are made from our server, not your device, and carry no account identifier: never your user ID, email, username, or name. See Section 3.3 for what we keep.
- Resend. Email delivery. Resend sends the confirmation email when you sign up for the FitCreature beta at devtegra.com/beta; sign-in, password, and email-change messages for accounts that sign in with an email address, such as partner portal logins; our internal alerts about partnership inquiries; and notification emails to the addresses a partner business chooses (see Section 5.4). Resend processes the recipient's email address and the message content only to deliver these messages. None of your FitCreature app data (your creature, stats, friends, workouts, or health data) is sent to Resend, and notification emails to partner businesses contain counts only, never who asked to join.
- Cloudflare (Turnstile). Our beta signup form and our partner inquiry form use Cloudflare Turnstile to tell humans from bots. Cloudflare receives the information needed to run that check (including your IP address and a challenge token). Turnstile is privacy-preserving; it is not used to track you across sites or to serve advertising.
...
FitCreature's Social tab can show cards for partner businesses (for example, a local gym) with their name, logo, website, locations, and in-store offers. **Which partner cards you see is determined only by your approximate region**: partners are shown either to everyone (national partners) or to users whose regional area is within a set distance of one of the partner's locations (see Section 8.2 on location). We compare only the center of your regional area, never your own position. No other signal is used: not your workouts, health data, purchases, demographics, or past activity in the app.

**What partner businesses can see.** A partner business never receives your email address, your username or display name, your location, your workouts or health data, or your level, streak, or stats. It can see:

- Aggregate counts about its own locations: how many times its signup page was opened (including, for a limited time, scans of printed codes at a location that has closed), how many accounts redeemed its code, how many of those redemptions were new signups, and how many people asked to join each of its locations. These counts contain no names, usernames, identifiers, or locations, and are not connected to any individual. The business sees them in its partner portal, and the join-request counts can also reach it in a morning email summary sent once a day.
- Information you choose to share by adding one of its locations as a friend in the app. When you ask to join, the business sees your creature's name and number (for example, Blaze#0042), your creature type, and the date you asked. If it accepts you, it sees the same details and the date you joined, and your creature can appear on the location's member board described in Section 5.3. Every staff login the business has for that location can see this, including managers of that location. The business can accept or remove you. You can leave at any time by removing the location from your friends list or blocking it, and you drop off the business's members list at once.

If a location you added closes, it is marked Closed in your friends list, and your connection to it is deleted automatically after a limited period (currently 30 days).

If we ever add aggregate counts of how often partner cards are viewed or tapped in the app, those will follow the same design: daily totals only, with no user, device, or session identifiers.

If you run a business that partners with FitCreature, or ask about partnering, we process: the business name and the contact name, email address, and phone number you give us when you ask about partnering; the name, sign-in email, role (brand admin or location manager), and assigned locations of each staff login you ask us to create or we create for your business, and each login's sign-in credentials; the notification email addresses you choose for your business and for each location (which can be a shared or group inbox) and your notification settings; the listing content, locations, addresses, map pins, and images you submit; your acceptance records for partner agreements (the time, network address, browser, the role of the person accepting, and the exact document version accepted); a log of actions taken in the portal, including which login made each change or downloaded a print pack or a signed agreement, with the time, network address, and browser; records of the notification emails we send (the address, when it was sent, what it said, and the email service's message reference); and aggregate performance counts for your own locations. We use each location's map pin to decide where your card appears and to work out the location's local time for morning email summaries. We use this information to operate the partner program. Acceptance records and the portal activity log are kept as legal and security records for the life of the partnership and as long as our legal obligations require. Records of notification emails are deleted after a limited period (currently 90 days).
...
- Your account, profile, game state (creature, battle stats, currency, cosmetics), friend connections (including any partner business locations you added, which also takes your creature off their members lists and member boards; see Section 5.3), push tokens, feedback, and subscription and gift records are deleted from our active systems as part of the deletion request.
- Purchase transaction records are retained with the link to your account removed, as the financial record we are required to keep, and the notices Apple and Google send us about purchases and subscriptions (such as a renewal or a refund) are retained with the kind of notice, the product, and the transaction references, and no link to your account (see the Data Deletion page for details). Deleting your account does not cancel a Premium subscription; you can cancel it in your device's subscription settings.
- Battle records, activity items, and currency records on other players' side are retained: battles you fought stay in your opponents' history, items on another player's activity feed that involved you (a gift you sent them, a thank-you you sent, a workout they tagged you in) stay on their feed, and a friend's record of a streak freeze they gifted you stays in their currency history, with your identity replaced by an anonymous "[deleted user]" placeholder that contains no information identifying you (see the Data Deletion page). The record of a reward another player earned in a battle against you, or when one of you used the other's invite code, also stays in their currency history, with nothing that points to you.
- Another player's app keeps what it last received from us (for example, your creature in their friends list) until it next connects to our servers, a notification already delivered to their phone stays there until they dismiss it, and a workout they logged with you keeps its tag, which no longer leads to your account.
- If you added a partner business location as a friend, our log of actions a partner business takes in its portal keeps the entries for accepting you as a member, declining your request to join, or removing you, with nothing that points to you (see Section 5.4).
...
- Backups containing your data may persist for up to 90 days before they are overwritten in the normal course of backup rotation. Our hosting provider's service logs, which can include your account ID, email address, and IP address, are deleted automatically after a limited period (currently 7 days).
- Anonymized or aggregated data that cannot be used to identify you may be retained for analytics and to run the Services: for example, counts such as how many times a code was redeemed, how many people asked to join a partner location, or how many members a Gym Zone has had.
- The Gym Zones you were placed in stay in the game, with their area and name and no link to you. For up to 30 days we also keep the place name we looked up for a Gym Zone's area and the coded fingerprint of a city or zip you searched for, both kept apart from any account (see Section 3.3).
- Limited records may be retained longer where required by law or needed to respond to legal claims or regulatory requests (for example, financial records for tax or fraud-prevention purposes, and our record of each privacy request you send us, such as a deletion request: your email address, your account ID, the type of request, the dates, and what we did).
- A beta program signup (see Section 3.1) is kept apart from your account and is not deleted with it: you can ask us to remove it at any time, and we delete the beta list once the beta program ends.

Data that lives only on your device (your workout log, exercise templates, equipment-setup preferences, health data, body metrics, and photos) is under your control: deleting the app, or deleting your account from within the app, removes it from your device. Workouts the app wrote to Apple Health or Health Connect for you (see Section 3.2) are kept by that service, not by FitCreature, and can stay there until you delete them there, and a file you exported or shared from the app stays wherever you saved or sent it. If your device's backup (iCloud or Android backup) was made before you deleted your account or the app, the copy of this data in that backup stays until the backup is replaced or you delete it. Progress photo image files are left out of those backups; each photo's details (such as its date, pose, notes, and the body weight and training goal recorded with it) are included.
...
You can disable location services for FitCreature at any time from your device settings. If disabled, you can still use the app; you just won't automatically join Gym Zones via location. You can join a zone manually using your city or zip code (the city or zip you type is used only to find your general area and is not stored with your account; see Section 3.3).
